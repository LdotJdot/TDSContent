...
namespace TDSContentApp.USN.Engine.Actions.USN
{
    public class NtfsUsnJournal : IDisposable
    {
        ...
        private bool EntryNameCheck(string name)
        {
            ...
            foreach (var ban in banWordsStartWith)
            {
                if (name.StartsWith(ban, StringComparison.OrdinalIgnoreCase))
                {
                    return false;
                }
            }
        ...
        private nint _usnJournalRootHandle;
        ...
        public NtfsUsnJournal(in DriveInfoData driveInfo)
        {
            ...
            if (0 == string.Compare(_driveInfo.DriveFormat, "ntfs", true))
            {
                ...
                UsnJournalReturnCode usnRtnCode = GetRootHandle(out nint rootHandle);
            ...
            CreateUsnJournal(ulong maxSize, ulong allocationDelta)
        {
            ...
            if (bNtfsVolume)
            {
                if (_usnJournalRootHandle.ToInt64() != Win32Api.INVALID_HANDLE_VALUE)
                {
                    ...
                    nint cujdBuffer = Marshal.AllocHGlobal(sizeCujd);
        ...
        ///Getall
        public void GetNtfsVolumeAllentries(string volname, out UsnJournalReturnCode usnRtnCode, ConcurrentDictionary<ulong, FileEntry> dict)
        {

            dict.TryAdd(ROOT_FILE_REFERENCE_NUMBER, new FileEntry(ROOT_FILE_REFERENCE_NUMBER, ulong.MaxValue, volname.TrimEnd('\\')));
                    ...
                    if (usnRtnCode == UsnJournalReturnCode.USN_JOURNAL_SUCCESS)
                    {
                            ...
                            IntPtr.Zero))
                        {
                            ...
                            while (outBytesReturned > 60)
                            {
                                ...
                                if (EntryNameCheck(usnEntry.Name))
                                {
                                    dict.TryAdd(usnEntry.FileReferenceNumber, new FileEntry(usnEntry.FileReferenceNumber, usnEntry.ParentFileReferenceNumber,usnEntry.Name));
                                }
            ...
            DeleteUsnJournal(Win32Api.USN_JOURNAL_DATA journalState)
        {
            ...
            if (bNtfsVolume)
            {
                if (_usnJournalRootHandle.ToInt64() != Win32Api.INVALID_HANDLE_VALUE)
                {
                    ...
                    nint dujdBuffer = Marshal.AllocHGlobal(sizeDujd);
            ...
            if (bNtfsVolume)
            {
                if (_usnJournalRootHandle.ToInt64() != Win32Api.INVALID_HANDLE_VALUE)
                {
                    if (frn != 0)
                    {
                        ...
                        nint refPtr = Marshal.AllocHGlobal(8);
                        nint objAttIntPtr = Marshal.AllocHGlobal(Marshal.SizeOf(objAttributes));
                        ...
                        if (fOk == 0)
                        {
                            ...
                            if (fOk == 0)
                            {
                                ...
                                //
                                path = Marshal.PtrToStringUni(new nint(buffer.ToInt64() + 4), nameLength / 2);
                            }
                        }
            ...
            if (bNtfsVolume)
            {
                if (_usnJournalRootHandle.ToInt64() != Win32Api.INVALID_HANDLE_VALUE)
                {
                    ...
                    if (usnRtnCode == UsnJournalReturnCode.USN_JOURNAL_SUCCESS)
                    {
                        ...
                        nint pbData = Marshal.AllocHGlobal(pbDataSize);
                        ...
                        nint rujdBuffer = Marshal.AllocHGlobal(sizeRujd);
                        ...
                        while (bReadMore)
                        {
                            ...
                            if (bRtn)
                            {
                                nint pUsnRecord = new nint(pbData.ToInt64() + sizeof(ulong));
                                while (outBytesReturned > 60)   // while there are at least one entry in the usn journal
                                {
                                    ...
                                    pUsnRecord = new nint(pUsnRecord.ToInt64() + usnEntry.RecordLength);
                                    outBytesReturned -= usnEntry.RecordLength;
            ...
            GetVolumeSerialNumber(in DriveInfoData driveInfo, out uint volumeSerialNumber)
        {
            ...
            nint hRoot = Win32Api.CreateFile(pathRoot,
                0,
        ...
        private UsnJournalReturnCode
            GetRootHandle(out nint rootHandle)
        {
            //
